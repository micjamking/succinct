		...
		var text = $('.truncate').html(),
			...
			colorful = function() {
				var hue = Math.floor(Math.random()*359),
					styles = '<style>.color {background-color: hsl(' + hue + ', 36%, 48%);color: hsl(' + hue + ', 36%, 48%);border-color: hsl(' + hue + ', 36%, 48%);}.color-light {border-color: hsla(' + hue + ', 100%, 48%,0.75);}.link:hover > span,.link:focus > span,.link:active > span,.link > span:after {background: hsl(' + hue + ', 36%, 48%);}input[type="submit"]:hover,input[type="submit"]:focus,.button{background-color: hsl(' + hue + ', 36%, 43%);}input[type="text"]:focus{box-shadow: 0 0 5px hsl(' + hue + ', 54%, 50%);}</style>';
